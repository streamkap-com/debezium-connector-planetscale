...
public class VitessMetadata {
    ...
    public List<String> getColumnsForTable(String keyspace, Tables.ColumnNameFilter columnFilter,
    ...
    public List<String> getKeyspaceTables(VitessConnectorConfig connectionConfig) {
    ...
    public List<String> getTableColumns(VitessConnectorConfig connectionConfig, String tableName) {
        String query = String.format(
                "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '%s' AND TABLE_NAME = '%s'",
                connectionConfig.getKeyspace(),
                tableName);
        Vtgate.ExecuteResponse response = executeQuery(query);
        List<String> columns = getFlattenedRowsFromResponse(response);
        LOGGER.info("All columns from table {} from keyspace {} are: {}", tableName, connectionConfig.getKeyspace(),
                columns);
        return columns;
    }

    public Instant getCurrentTimestamp(VitessConnectorConfig connectionConfig) {
        Vtgate.ExecuteResponse response = executeQuery("SELECT CURRENT_TIMESTAMP");
        List<Timestamp> timestamps = getRowsFromResponse(response).stream().map(row -> {
            if (row.isEmpty()) {
                return null;
            }
            String fieldValue = row.get(0);
            return Timestamp.valueOf(fieldValue);
        }).collect(Collectors.toList());

        // If no timestamps are returned, return null
