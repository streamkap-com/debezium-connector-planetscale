...
public class VitessReplicationConnectionIT {
    ...
    public void shouldErrorOutWhenSkipEnabled() throws Exception {
        ...
        connection.startStreaming(
                VitessOffsetContext.initialContext(false, conf, Clock.system()),
                (message, vgtid) -> {
                    if (!started.get()) {
                        started.set(true);
                    }
    ...
    public void shouldErrorOutWhenWarnEnabled() throws Exception {
        ...
        BlockingQueue<MessageAndVgtid> consumedMessages = new ArrayBlockingQueue<>(100);
        AtomicBoolean started = new AtomicBoolean(false);
        TransactionContext transactionContext = conf.getTransactionMetadataFactory().getTransactionContext();
        VitessOffsetContext context = new VitessOffsetContext(false, false, conf, startingVgtid, Clock.system().currentTimeAsInstant(), transactionContext);
        connection.startStreaming(
                context,
                (message, vgtid) -> {
                    if (!started.get()) {
